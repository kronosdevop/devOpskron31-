    ...
    >
      ...
      <template #[`item.action`]="{ item }">
        <v-btn
          ...
          size="x-small"
          color="green"
          @click="OpenDialogScheduleGame(item)"
        >
          Schedule
        </v-btn>
      </template>
    ...
    <div class="table-footer">
      <div class="footer-info">
        {{
          totalItems === 0
            ? "No Results"
            : `Showing ${paginatedItems.length} of ${totalItems} default categories`
        }}
      </div>
...
export default {
  ...
  components: { ScheduleGameDialog, ScheduledGames, SnackBar },
  ...
  computed: {
    totalItems() {
      return this.ListAllCategories.length;
    },
    paginatedItems() {
      const start = (this.currentPage - 1) * this.itemsPerPage;
      const end = start + this.itemsPerPage;
      return this.ListAllCategories.slice(start, end);
    },
    pageCount() {
      return Math.ceil(this.ListAllCategories.length / this.itemsPerPage);
    },
    visiblePages() {
  ...
  async mounted() {
    this.windowHeight = window.innerHeight - 243;
    await this.GetAllCategoriesMethod();
  },

  methods: {
    OpenDialogScheduleGame(item) {
      this.StoreObj = item;
      this.DialogScheduleGame = true;
    },
    DialogScheduleGameEmit() {
