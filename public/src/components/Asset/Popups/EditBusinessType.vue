<template>
  <div>
    <!-- eslint-disable -->
    <v-dialog
      :model-value="editBusinessType"
      @update:model-value="editBusinessType = $event"
      persistent
      max-width="500"
      ...
      <v-card>
        <v-toolbar elevation="0" dark density="compact" class="navBar">
          <v-toolbar-title class="ml-2">
            <div class="custom-title">Edit Business Type</div>
          </v-toolbar-title>
          <v-spacer />
...
export default {
  ...
  watch: {
    editBusinessType: {
      handler(newVal) {
        if (newVal && this.businessTypeItem) {
          this.businessTypeName = this.businessTypeItem.bussinesstypes_name || "";
        }
      },
      immediate: true,
    },
    ...
    async update_mutation() {
      ...
      try {
        ...
        if (response.Status == "SUCCESS") {
          ...
          this.$emit("errorMsg", response.Message || "Failed to update business type");
        }
      } catch (error) {
        this.loading = false;
        this.$emit("errorMsg", error.errors?.[0]?.message || "Error updating business type");
      }
    },
  },
};
