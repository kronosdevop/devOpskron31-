<template>
  <div>
    <!-- eslint-disable -->
    <v-dialog
      :model-value="handoveringasset"
      @update:model-value="handoveringasset = $event"
      persistent
      max-width="410"
      ...
      <v-card>
        <v-toolbar elevation="0" dark density="compact" class="navBar">
          <v-toolbar-title class="ml-2">
            <div class="custom-title">Asset Handover</div></v-toolbar-title
          >
          <v-spacer />
          <v-icon class="icon-class mr-3" @click="close_dialog()"
            >mdi-close</v-icon
          >
        </v-toolbar>
        <v-card-text class="mt-2">
          <div class="text-left">
            Are you sure you want to handover the asset
            <b>{{ assetitems.asset_name }}</b> ?
          </div>
        </v-card-text>
