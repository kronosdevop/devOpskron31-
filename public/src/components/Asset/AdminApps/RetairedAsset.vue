  ...
  <div>
    <v-card flat class="">
      ...
      <v-card-text class="pa-4">
        <v-data-table :headers="adminassetHeaders" :items="paginatedItems" :loading="tableLoading"
          :sort-by="['asset_prefix']" :items-per-page="itemsPerPage" :page="currentPage"
          :server-items-length="totalCount" hide-default-footer class="modern-data-table" density="comfortable" hover
          :fixed-header="true" :height="windowHeight" @click:row="handle_row_click">
          ...
          <template v-slot:[`item.asset_status`]="{ item }">
            <span :class="item.asset_status == 'ACTIVE' ? 'green--text' : 'red--text'
          ...
          <template v-slot:[`item.asset_tagged_status`]="{ item }">
            <div v-if="item.asset_tagged_status == 'UNTAGGED'">
              <v-tooltip bottom>
                <template v-slot:activator="{ on, attrs }">
                  <v-icon size="small" color="red" v-bind="attrs" v-on="on">mdi-alpha-x-circle</v-icon>
                </template>
                <span>Untagged</span>
              </v-tooltip>
            </div>
            <div v-else>
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on, attrs }">
                  <v-icon size="small" color="green" v-bind="attrs" v-on="on"
                    @click.stop="open_qrDailog(item)">mdi-check-circle</v-icon>
                </template>
                <span>Tagged</span>
              </v-tooltip>
            </div>
    ...
    <div v-if="componentCheck == 1">
      <AdminAssetdetails :assetdisplay="assetdisplay" @clicked="assetdisplay = false"
