  ...
  <div>
    ...
    <v-app-bar class="modern-header-section">
      <div class="header-left">
        ...
        <div class="header-text">
          ...
          <span class="header-subtitle"
            >Manage vendor transactions and approvals</span
          >
        </div>
      </div>
      <v-spacer />
      <div class="header-actions">
        <!-- Filter Button -->
        <v-btn
          v-if="toggle_exclusive == 'vendor_transaction'"
          :class="
        ...
        <v-menu offset-y>
          <template #activator="{ props }">
            <v-btn
              ...
              class="action-btn"
              size="small"
              v-if="toggle_exclusive == 'vendor_list'"
            >
          ...
          <v-list density="compact">
            ...
            <!-- Add Prefix - Only show for Vendors tab -->
            <!-- <v-list-item 
              v-if="toggle_exclusive == 'vendor_list'"
              @click="Add_prefix()"
              style="font-size: 12px"
            >
              ...
              <v-list-item-title>{{ prefix ? "Edit Prefix" : "Add Prefix" }}</v-list-item-title>
            </v-list-item> -->
            ...
            <v-list-item 
              v-if="toggle_exclusive == 'vendor_Report'"
    ...
    <v-toolbar flat class="mt-n10" style="background-color: white">
      <template v-slot:extension>
        <v-tabs v-model="toggle_exclusive" mandatory class="modern-tab-toggle">
          ...
          <v-tab value="expense_approval" class="tab-btn"> Approvals </v-tab>
          <v-tab value="vendor_Report" class="tab-btn"> Reports </v-tab>
          <v-tab value="vendor_list" class="tab-btn"> Vendors </v-tab>
        </v-tabs>
      </template>
    </v-toolbar>
    ...
    <v-card flat class="overflow-y-auto">
      ...
      <div class="text-left" v-if="toggle_exclusive == 'vendor_transaction'">
        <VendorTransaction
      ...
      <div class="text-left" v-if="toggle_exclusive == 'vendor_Report'">
        <VendorReportsub />
      </div>
      <div class="text-left" v-if="toggle_exclusive == 'vendor_list'">
        <AssetSuppliersList />
      </div>
    </v-card>
...
import AddVendorPrefix from "@/components/Asset/Popups/AddVendorPrefix.vue";
...
export default {
  ...
  data: () => {
    ...
    return {
      toggle_exclusive: "vendor_transaction",
      componentCheck: 0,
  ...
  methods: {
    ...
    export_data() {
      ...
      if (this.toggle_exclusive == 'vendor_list') {
        this.export_vendor();
      } else {
        this.SnackBarComponent = {
          SnackbarVmodel: true,
          SnackbarColor: "green",
          SnackbarText: "Export functionality will be implemented based on current tab.",
          timeout: 5000,
          Top: true,
        };
      }
    ...
    bulk_import() {
      // Bulk import functionality - handled by BulkIndex component
      this.SnackBarComponent = {
        SnackbarVmodel: true,
        SnackbarColor: "green",
        SnackbarText: "Bulk import functionality is available in the Vendors tab.",
        timeout: 5000,
        Top: true,
      };
    },
    ...
    handleSuccessbusiness({ results, header }) {
      ...
      //     SnackbarVmodel: true,
      //     SnackbarColor: "error",
      //     SnackbarText: "Please add prefix",
      //     timeout: 5000,
      //     Top: true,
      //   };
      //   return;
