<template>
  ...
  <div>
    <v-dialog
      :model-value="memberRemove"
      @update:model-value="$emit('update:memberRemove', $event)"
      persistent
      max-width="600"
      ...
      <v-card>
        <v-toolbar elevation="0" dark density="compact" class="navBar">
          <v-toolbar-title class="text--black ml-2"
            ><div class="custom-title">Remove Member</div></v-toolbar-title
          >
          <v-spacer />
  ...
  methods: {
    ...
    async fetch_audit_message() {
      ...
      var data = {
        ...
        AuditMessage: `The Remove Member Workflow for ${this.rowInfo.full_user_name} has been initiated by ${data.user.full_user_name}`,
      };
