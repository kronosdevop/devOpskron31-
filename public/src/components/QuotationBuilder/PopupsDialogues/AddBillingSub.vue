<template>
  <div>
    <!--  eslint-disable  -->
    <v-dialog
      :model-value="AddBillingSubDialog"
      @update:model-value="$emit('update:AddBillingSubDialog', $event)"
      persistent
      max-width="400"
...
export default {
  ...
  watch: {
    AddBillingSubDialog: {
      async handler() {
        if (this.AddBillingSubDialog == true) {
          this.price = this.selectedSubCategoryValue.sub_category_price;
          this.gstTax = this.selectedSubCategoryValue.sub_category_tax;
