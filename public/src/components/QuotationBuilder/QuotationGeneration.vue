<template>
  <div>
    ...
    >
      <v-toolbar-title>Generate Quotation</v-toolbar-title>
          ...
          <v-row no-gutters class="ma-2">
            <v-col cols="4">
              <v-autocomplete
                ...
                @input="fetch_entity(selectEntity)"
                return-object
              ></v-autocomplete>
            </v-col>
            <v-spacer />
...
import PreviewDialogue from "@/components/QuotationBuilder/PopupsDialogues/PreviewDialogue";
...
export default {
  ...
  async created() {
    ...
    this.termCondtion =
      this.$store.getters.GetTermsandConditions.data[0].quotation_terms_conditions;
  },
  ...
  methods: {
    ...
    fetch_selected_category() {
      ...
      this.subMasterList.forEach((element) => {
        // Check if the selected category matches the element's category_id
