      ...
      <v-card>
        ...
        <v-card-text v-if="deletedata" class="mt-4">
          <div class="text-center">
            ...
            <strong>{{ deletedata.category_name }} Category</strong> ?
          </div>
        </v-card-text>
...
export default {
  ...
  data() {
   ...
   computed: {
  categoryName() {
    return this.deletedata?.category_name || "";
      ...
      try {
        let result = await API.graphql(
          graphqlOperation(inventory_categories_CRUD, {
            input: {
              category_id: this.deletedata.category_id,
              action_type: "DELETE",
            },
          })
        );
        var response = JSON.parse(result.data.inventory_categories_CRUD);
