      ...
      <v-card>
        ...
        <v-card-text v-if="deletedata" class="mt-4">
          <div class="text-center">
            ...
            <strong>{{ deletedata.material_unit_name }} unit</strong> ?
          </div>
        </v-card-text>
...
export default {
  ...
  data() {
   ...
   computed: {
  UnitName() {
    return this.deletedata?.material_unit_name || "";
      ...
      try {
        let result = await API.graphql(
          graphqlOperation(create_list_edit_delete_material_unit, {
            input: {
              material_unit_id: this.deletedata.material_unit_id,
              action_type: "DELETE",
            },
          })
        );
        var response = JSON.parse(result.data.create_list_edit_delete_material_unit);
