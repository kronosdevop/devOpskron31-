<template>
  <div>
    <v-toolbar class="mt-4 bg-white" density="compact" flat>
      <v-toolbar-title> Design Form for {{ currentFormName }} </v-toolbar-title>
      <v-spacer />
      <v-btn
              ...
              <template v-for="(item, i) in items" :key="i">
                ...
                >
                  ...
                  <v-list-item-action
                    v-if="
                      formCheck &&
                      selectedFormValue?.form_name === item.form_name
                    "
                    class="justify-end mt-n8"
                  >
                    <v-btn
                      size="x-small"
                      @click.stop="use_template(item)"
                      color="primary"
                    >
                      Use Template
                    </v-btn>
...
>
  ...
  <v-tab v-if="formType == 'DEFAULT'">
    <v-icon size="16" class="mr-1">mdi-cog-outline</v-icon>
    System
  </v-tab>

  <v-tab>
    <v-icon size="16" class="mr-1">mdi-image-outline</v-icon>
    Media
  </v-tab>
</v-tabs>
                ...
                <v-row no-gutters>
                  ...
                  >
                    <v-btn
                      shaped
                      block
class="ma-2 field-btn"                      @click="fetch_selected_item(item)"
                    >
                      <v-row justify="start">
                        <v-icon left small>{{ item.icon }}</v-icon>
                        <div class="text-left" style="font-size: small">
...
class="ma-2 field-btn"                      @click="fetch_selected_item(item)"
    ...
    <v-dialog v-model="showDeleteAllDialog" max-width="400">
      <v-card>
        <v-card-title class="headline"> Confirm Delete All </v-card-title>
        <v-card-text>
          Are you sure you want to delete all options? This action cannot be
          undone.
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="grey" text @click="showDeleteAllDialog = false">
            Cancel
          </v-btn>
          <v-btn color="error" text @click="clearAllOptions">
            Delete All
          </v-btn>
        </v-card-actions>
      </v-card>
...
import { design_worktype } from "@/graphql/mutations.js";
...
export default {
  ...
  async mounted() {
    // ✅ FORM NAME
    this.currentFormName = this.$store.getters.GetFormName || "";

    // ✅ FORM ID
    this.worktype_form_id =
      this.$store.getters.GetWorktypeFormId.worktype_form_id;
    console.log("WorkFormId", this.worktype_form_id);

    // ✅ GET STORE DATA
    const formData = this.$store.getters.GetFormObject;

    console.log("FORM DATA:", formData);

    // 🔥 STEP 1: UNIFIED TEMPLATE LOAD
    if (formData) {
      let template =
        formData.worktype_form_design_template || formData.form_design_template;
      ...
      // 🔥 STEP 4: SET UI (DEEP COPY)
      this.formFields = JSON.parse(JSON.stringify(template));
    } else {
      this.formFields = [];
    }

    // ✅ FORM TYPE (KEEP EXISTING LOGIC)
    ...
    // ✅ LIBRARY MODE (DO NOT TOUCH formFields)
    if (this.$store.getters.GetFormLibrary == true) {
      ...
      let formarray = this.globalForms;
      let dataform = this.$store.getters.Getlibraryformtype;
  ...
  methods: {
    ...
    updateitems(val) {
      ...
      const mergedOptions = [
        ...new Set([...this.globalOptions, ...finalArray]),
      ];

      if (this.editable) {
        // Preserve reference when editing so fieldOptions updates reactively
        this.globalOptions.splice(
          0,
          this.globalOptions.length,
          ...mergedOptions,
        );
    ...
    get_selected_form_item(item) {
      // ✅ mark selection active
      this.formCheck = true;

      // ✅ store selected item (reactive safe copy)
    ...
    use_template(item) {
      let template =
        item.worktype_form_design_template || item.form_design_template;
      ...
      // 🔥 FIX 1: correct ID mapping (VERY IMPORTANT)
      const formId = this.$store.getters.GetWorktypeFormId.worktype_form_id;
      console.log("formidentity", formId);
      // this.$store.commit("SetWorktypeFormId", formId);

      this.$store.commit("SetFormObj", {
        worktype_form_design_template: template,
        is_form_designed: true,
      });

      // ✅ deep copy (prevent mutation bugs)
      this.formFields = JSON.parse(JSON.stringify(template));
    ...
    async save_directory_form() {
      ...
      console.log("FINAL FORM ID:", formId);

      if (!formId) {
        this.SnackBarComponent = {
          SnackbarVmodel: true,
          SnackbarColor: "red",
          SnackbarText: "❌ Form ID missing",
          timeout: 5000,
          Top: true,
        };
        this.loading = false;
      ...
      try {
        const result = await API.graphql(
          graphqlOperation(design_worktype, {
            input: {
              organization_id: data.organization.organization_id,

              // ✅ KEEP DEFAULT (as you said)
              action_type: "DEFAULT",

              // 🔥 FIXED LINE
              worktype_form_id: formId,

              worktype_form_template: JSON.stringify(this.formFields),
              worktype_form_design_template: JSON.stringify(this.formFields),
            },
          }),
        );

        const response = JSON.parse(result.data.design_worktype);

        if (response.Status == "SUCCESS") {
          this.SnackBarComponent = {
            SnackbarVmodel: true,
            SnackbarColor: "green",
            SnackbarText: "✅ Form Saved successfully!",
            timeout: 4000,
            Top: true,
          };

          this.$store.commit("SetWorktypeFormId", "");
        ...
        this.SnackBarComponent = {
          SnackbarVmodel: true,
          SnackbarColor: "red",
          SnackbarText: error?.errors?.[0]?.message || "API Error",
          timeout: 5000,
          Top: true,
        };
      } finally {
      ...
      try {
        const result = await API.graphql(
          graphqlOperation(design_worktype, {
            input: {
              organization_id: data.organization.organization_id,
              action_type: "CUSTOM",
              worktype_form_id: this.worktype_form_id,
              worktype_form_template: JSON.stringify(this.formFields),
              worktype_form_design_template: JSON.stringify(this.formFields),
            },
          }),
        ...
        if (response.Status == "SUCCESS") {
          this.SnackBarComponent = {
            SnackbarVmodel: true,
            SnackbarColor: "green",
            SnackbarText: "✅ Form Created successfully!",
            timeout: 4000,
            Top: true,
          };
          this.$store.commit("SetWorktypeFormId", "");
...
.cardCss {
  background: linear-gradient(135deg, #dd4d78, #f06292) !important;
...
.cardCss:hover {
  ...
  box-shadow: 0 8px 20px rgba(221, 77, 120, 0.4);
}
...
/* HOVER EFFECT */
:deep(.v-tab:not(.v-tab--selected):hover) {
  background: #ffe4ec;
}
.field-btn {
...
.field-btn:hover {
  background: linear-gradient(135deg, #fff1f5, #ffe4ec) !important;
  border-color: #dd4d78;
  color: #dd4d78;
  transform: translateY(-2px);
  box-shadow: 0 6px 14px rgba(221, 77, 120, 0.2);
}
.add-btn {
  background: linear-gradient(135deg, #dd4d78, #f06292) !important;
  border-radius: 10px;
...
:deep(.v-field--variant-outlined .v-label) {
  background: #fff !important; /* MUST match input background */
  padding: 0 6px;
}
...
/* REMOVE EXTRA LEFT SPACE FROM ICON */
:deep(.v-icon) {
  margin-right: 4px !important;
}

/* ENSURE EVEN TAB WIDTH */
