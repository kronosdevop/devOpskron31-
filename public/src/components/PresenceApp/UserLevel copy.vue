<template>
  <div class="presence-container">
      ...
      <div class="header-actions">
        ...
        <v-menu offset-y>
          <template v-slot:activator="{ props }">
            <v-btn class="action-btn menu-btn" v-bind="props" size="small">
          ...
          <v-list density="compact">
            ...
            <v-list-item
              v-if="toggle_exclusive === 'reportuser'"
              @click="monthPicker = true"
              style="font-size: 12px"
            >
              <template #prepend>
                <v-icon size="small" color="purple">mdi-calendar-month</v-icon>
              </template>
              <v-list-item-title>{{ datedisplay }}</v-list-item-title>
            </v-list-item>
    ...
    <div class="text-left" v-if="toggle_exclusive === 'reportuser'">
      <UserReports :key="monthpick" :monthvalue="monthvalue" />
    </div>
...
export default {
  ...
  methods: {
    ...
    populateYearItems() {
      const organizationTimestamp =
        this.$store.getters.GetOrgDetails.organization.organization_created_on;
    ...
    async get_datails() {
      ...
      try {
        await API.graphql(
          ...
          this.is_early_salary_enabled = DataItems.is_earlysalary_enabled;
        });
      } catch (error) {
        this.overlay = false;
        this.SnackBarComponent = {
      ...
      await this.fetchData(get_presence_dashboard, variables)
        .then(() => {
          ...
          if (response.Status == "SUCCESS") {
            ...
            // console.log('API Response - all_dates_details:', result);
            // console.log('Date Elements set:', this.dateElements);
  ...
  computed: {
    ...
    eventsFunction() {
      return (date) => {
        if (!this.dateElements || this.dateElements.length === 0) {
          // console.log('No dateElements available for events');
          return false;
        }

        // Convert the date parameter to YYYY-MM-DD format for comparison
        const dateToCheck = new Date(date);
        const dateString = dateToCheck.toISOString().split("T")[0];

        // Check if this date exists in our dateElements
    ...
    eventColorFunction() {
      return (date) => {
        if (!this.dateElements || this.dateElements.length === 0) {
          // console.log('No dateElements available for event colors');
          return null;
        }
        ...
        if (event) {
          // console.log('Color event found for date:', dateString, 'Type:', event.attendence_type);
          // Return color based on attendance type
