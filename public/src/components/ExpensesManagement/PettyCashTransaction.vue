          ...
          <template v-slot:no-data>
            <div class="text-center py-8">
              ...
              <div class="text-body-2 text-grey">
                No petty cash records available at the moment.
              </div>
            </div>
          </template>
            ...
            <div class="d-flex align-center">
              ...
              <div>
                <div class="font-weight-medium text-body-2">
                  ₹{{ getAmountValue(item) }}
                  <v-chip
                    v-if="item.entry_type === 'REFILL'"
                    size="x-small"
                    color="success"
                    class="ml-2"
                    variant="outlined"
                  >
                    <v-icon size="12" class="mr-1">mdi-plus-circle</v-icon>
                    REFILL
                  </v-chip>
                </div>
              </div>
          ...
          <template v-slot:[`item.transaction_status`]="{ item }">
            <div class="d-flex align-center">
              <v-icon
                ...
                size="20"
                class="mr-3"
                :icon="getStatusIcon(item.transaction_status)"
              ></v-icon>
              <span
...
<script>
import { formatedatetime, formatdisplayDate } from "@/JsonFiles/DateFormate.js";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectTransactionType: "PETTY_CASH",
      SnackBarComponent: {},
      tableLoading: false,
      componentCheck: 0,
      height: 0,
  ...
  methods: {
    ...
    getAmountValue(item) {
      // If entry_type is REFILL, use granted_amount, otherwise use amount
      if (item.entry_type === "REFILL") {
        return item.granted_amount || "0";
      }
    ...
    async get_initiated_expense() {
      ...
      try {
        let result = await API.graphql(
          graphqlOperation(list_expense_transactions, {
            input: {
              ...
              transaction_status: this.selectStatusType == 'ALL' ? undefined : this.selectStatusType,
        ...
        if (response.Status == "SUCCESS") {
          ...
          // console.log("PettyCashTransaction Response:", response);
