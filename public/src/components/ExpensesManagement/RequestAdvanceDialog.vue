<template>
  <v-dialog :model-value="advanceDialog" max-width="420">
    <v-card class="pa-0 rounded-xl">
      <v-card-title class="d-flex align-center justify-space-between px-6 py-4">
        <span class="font-weight-bold text-h5">Request Advance</span>
        ...
        try {
          let result = await API.graphql(
            graphqlOperation(request_and_log_pettycash, {
              input: {
                action_type: "PETTY_CASH_ADVANCE",
                // expense_group_id: this.formGroup,
