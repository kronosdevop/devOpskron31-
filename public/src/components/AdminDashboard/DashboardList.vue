    ...
    <div v-if="dispalyadashboard == false">
      <v-card flat>
        <v-toolbar class="mt-4" dense flat>
          <!-- <v-toolbar-title>Dashboard  Management</v-toolbar-title> -->
          <v-spacer />
          <v-btn
            depressed
            @click="add_mutation()"
        ...
        <v-card-text>
          ...
          >
            <!-- @click:row="handleClick"   :height="height" -->
              ...
              <v-tooltip bottom>
                <template #activator="{ on }">
                  ...
                  <div class="ml-7 mt-n4" v-on="on">
                    {{
                      item.dashboard_name.length >= 20
                        ? item.dashboard_name.substr(0, 20) + "..."
                        : item.dashboard_name
                    }}
                  </div>
                </template>
...
export default {
  ...
  methods: {
    ...
    //   if (itemm.title == "View Details") {
    //     this.custom_details = item;
    //     this.edit_properties = true;
    //   }
    //   if (itemm.title == "Delete") {
    //     this.custom_details = item;
    //     this.delete_customdashboard = true;
    //   }
    //   if (itemm.title == "View Qr") {
    //     this.custom_details = item;
    //     this.qr_view = true;
    //   }
    // },
    ...
    async fetch_dashboard_details() {
      ...
      try {
        ...
        const uniqueArrayOfObjects = this.teamData.filter(
          (obj, index, self) =>
            index === self.findIndex((o) => o.dashboard_id === obj.dashboard_id)
        );

        this.teamData = uniqueArrayOfObjects;

        this.teamData = response.items;
