<template>
  ...
  <div>
    <v-dialog
      :model-value="memberadd"
      @update:model-value="$emit('update:memberadd', $event)"
      persistent
      max-width="500"
        ...
        <v-toolbar elevation="0" dark dense class="navBar">
          <v-toolbar-title class="text--black ml-2"
            ><div class="custom-title">
              Add Member for {{ dashboarditems.dashboard_name }}
            </div>
          </v-toolbar-title>
          <v-spacer />
    ...
    fetch_details() {
      this.orgUsers.forEach((element) => {
        if (element.user_status == "ACTIVE" && element.user_type != "CHATBOT") {
          if (
            !this.tableData.some(
              (user) => user.client_id === element.user_email_id
            )
          ) {
            this.userArray.push({
