      ...
      <v-card>
        ...
        <v-card-text class="mt-6">
          <div class="text-center">
            ...
            <strong>{{ rowInfo.designation_name }}</strong> ?
          </div>
        </v-card-text>
...
<script>
import { activate_and_reactivate_designations } from "@/graphql/mutations.js";
    ...
    async delete_mutation() {
      ...
      try {
        let result = await API.graphql(
          graphqlOperation(activate_and_reactivate_designations, {
            designation_id: this.rowInfo.designation_id,
            designation_status: "DEACTIVE",
          })
        );

        var response = JSON.parse(
          result.data.activate_and_reactivate_designations
        );
