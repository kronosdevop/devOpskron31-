        ...
        <v-card-text>
          <v-form ref="form">
            <!-- <v-row no-gutters class="mt-4">
                <v-col cols="6">
                  <v-radio-group
                    @update:modelValue="fetch_selected_data(expenseHead, 'initator')"
                    v-model="initiator"
                    row
                    color="primary"
                  >
            ...
            <!-- <v-row no-gutters>
                <v-col cols="6">
                  <v-radio-group
                    @update:modelValue="fetch_selected_data(expenseHead, 'expense')"
                    v-model="expenseHead"
                    row
                    color="primary"
                  >
...
<script>
/*eslint-disable*/
import { get_awards_workflows } from "@/graphql/queries.js";
import { API, graphqlOperation } from "aws-amplify";

import { get_all_org_users } from "@/mixins/GetUsersDropdown.js";

import OverlayComp from "@/components/OverlayComp.vue";
import FormConfiguration from "@/components/RewardsManagement/FormConfiguration.vue";
...
export default {
  ...
  methods: {
    ...
    fetchclear() {
      ...
      this.initiator = null;
      // Don't clear initiator when team member is selected
      // this.initiator = null;
    },
    ...
    async get_initiated_expense() {
      ...
      try {
        let result = await API.graphql(
          graphqlOperation(get_awards_workflows, {
            input: {
        ...
        var response = JSON.parse(result.data.get_awards_workflows);
        this.overlay = false;
        this.tableData = {};
        ...
        if (response.Status == "SUCCESS") {
          ...
          //     if (this.addedRecords[s].user_type == "team_member") {
