...
export default {
  ...
  async mounted() {
    try {
      this.editor12 = await ClassicEditor.create(
        document.querySelector("#editor12"),
        {
          toolbar: {
            items: [
              "undo",
      ...
      // Set initial value
      this.editor12.setData(this.bodyValue);

      // Listen for changes and update bodyValue
      this.editor12.model.document.on("change:data", () => {
        this.bodyValue = this.editor12.getData();
      });
    } catch (error) {
  ...
  methods: {
    ...
    insertSelectedItem() {
      if (this.selectedItem) {
        this.editor12.model.change((writer) => {
          writer.insertText(
            this.selectedItem,
            this.editor12.model.document.selection.getFirstPosition()
          );
        });
        this.bodyError = "";
      }
    ...
    async fetch_templatelist() {
      try {
        ...
        if (response.Status === "SUCCESS" && response.data.length > 0) {
          ...
          if (this.editor12) {
            this.editor12.setData(emailTemplate.body || "");
          } else {
            this.$nextTick(() => {
              if (this.editor12) {
                this.editor12.setData(emailTemplate.body || "");
    ...
    validate_actions() {
      const emailBody = this.editor12.getData().trim(); // Get editor content
