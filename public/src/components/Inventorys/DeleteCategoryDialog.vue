<template>
  ...
  <div>
    <v-dialog
      :model-value="deleteCategoryDialog"
      @update:model-value="$emit('update:deleteCategoryDialog', $event)"
      persistent
      max-width="400"
    ...
    >
      <v-card>
        ...
        <v-card-text v-if="categoryItem" class="mt-4">
          <div class="text-center">
            Are you sure you want to delete the
            <strong>{{ categoryItem.category_name }} Category</strong> ?
          </div>
        </v-card-text>
...
export default {
  ...
  data() {
   ...
   computed: {
  categoryName() {
    return this.categoryItem?.category_name || "";
      ...
      try {
        ...
        let result = await API.graphql(
          graphqlOperation(items_categories, {
            input: {
              category_id: this.categoryItem.category_id,
              action_type: "DELETE",
            },
          })
        );
