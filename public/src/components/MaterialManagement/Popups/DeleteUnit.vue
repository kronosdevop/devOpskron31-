<template>
  <div>
    <!-- eslint-disable -->
    <v-dialog
      :model-value="unitdeletion"
      @update:model-value="$emit('update:unitdeletion', $event)"
      persistent
      max-width="400"
      ...
      <v-card>
        <v-toolbar elevation="0" dark density="compact" class="bg-white">
          <!-- <v-toolbar-title class="ml-2">
              <div class="custom-title">Delete Unit</div></v-toolbar-title
            > -->
          <v-spacer />
        ...
        <v-card-text class="mt-6">
          <div class="text-center">
            ...
            <b> {{ rowInfo.material_unit_name }} </b> ?
          </div>
        </v-card-text>
        ...
        let result = await API.graphql(
          graphqlOperation(create_list_edit_delete_units, {
            input: {
              ...
              material_unit_id: this.rowInfo.material_unit_id,
            },
          })
        );
