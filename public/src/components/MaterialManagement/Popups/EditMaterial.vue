<template>
  <div>
    <!-- eslint-disable -->
    <v-dialog :model-value="materialedition" @update:model-value="$emit('update:materialedition', $event)" persistent max-width="500" transition="dialog-top-transition">
      <v-card>
        <v-toolbar elevation="0" dark dense class="navBar">
          <v-toolbar-title class="text--black">
            <div class="custom-title">Edit Material</div></v-toolbar-title
          >
          <v-spacer />
    ...
    materialedition: {
      async handler() {
        if (this.materialedition == true) {
          ...
          //   "material_unit",
          //   JSON.stringify(this.rowInfo.material_unit)
          // );
          this.unitName = this.rowInfo.material_unit_id;
          this.materialName = this.rowInfo.material_name;
          // this.materialunit = JSON.parse(localStorage.getItem("material_unit"));
          this.supplierName =
            this.rowInfo.supplier_name == undefined
              ? ""
              : this.rowInfo.supplier_name;
        }
      },
      immediate: true,
    },
        ...
        let result = await API.graphql(
          graphqlOperation(create_list_edit_delete_material, {
            input: {
              ...
              material_id: this.rowInfo.material_id,
              action_type: "UPDATE_MATERIAL",
              material_name: this.materialName,
              material_unit_id: this.unitName,
              supplier_name: this.supplierName == null ? "" : this.supplierName,
            },
          })
        );
