    ...
    <v-dialog :model-value="requestAccessDialog" @update:model-value="$emit('update:requestAccessDialog', $event)" persistent max-width="450" transition="dialog-top-transition">
      <v-card>
        ...
        <v-card-text class="mt-4">
          <div class="text-left">
            Initiate a workflow to request access to this directory
...
<script>
/* eslint-disable */
import { request_directory_access } from "@/graphql/mutations.js";
    ...
    async request_access() {
      ...
      try {
        let result = await API.graphql(
          graphqlOperation(request_directory_access, {
            input: {
        ...
        this.loading = false;
        var response = JSON.parse(result.data.request_directory_access);
