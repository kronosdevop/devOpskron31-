  ...
  <div>
    ...
    >
      <v-card class="pa-0 rounded-xl">
        ...
        >
          <span class="font-weight-bold text-h5">Create Purchase Order</span>
        ...
        <v-card-text class="px-4 py-3">
          <v-form ref="form">
            <!-- Order & Supplier Details Section -->
            <v-row class="mb-4">
              <!-- Left Column - Supplier Details -->
              <v-col cols="12" md="3">
                <div class="mb-3">
                  <v-autocomplete
                    v-model="orderForm.supplier_search"
                    :items="supplierItems"
                    ...
                    :filter="customFilter"
                    @update:model-value="
                      onSupplierSelect(orderForm.supplier_search, 'edit')
                    "
                  />
                </div>
                <div class="mb-3">
...
export default {
    ...
    dialog: {
      async handler(newVal) {
        if (newVal) {
          ...
          // Load suppliers, inventory items, material units, and global categories when dialog opens
  ...
  methods: {
      ...
      if (valid) {
        ...
        try {
          ...
          const response = await API.graphql(
            graphqlOperation(create_purchase_order, { input })
          );
          // console.log("Response:", response);
          let result = JSON.parse(response.data.create_purchase_order);
    ...
    populateFormFromQuotation(quotationData) {
      ...
      this.orderForm.supplier_search = quotationData.supplier_name;
      this.orderForm.supplier_id = quotationData.supplier_id;
      this.orderForm.phone_no = quotationData.supplier_phone_no || "";
