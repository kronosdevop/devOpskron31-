      ...
      <v-card>
        ...
        <v-card-text v-show="actionType == 'Delete'" class="mt-4">
          ...
          <div class="text-left">
            ...
            <b>{{ rowInfo.full_user_name }}</b> from
            {{ selectedChannel.team_name }} channel ?
          </div>
        </v-card-text>
...
export default {
  ...
  methods: {
      ...
      try {
        let result = await API.graphql(
          graphqlOperation(add_remove_user_to_team, {
            input: {
              admin_user_id: this.selectedChannel.team_owner_id,
              organization_id: data.organization.organization_id,
              team_id: this.selectedChannel.team_id,
              action_type: this.actionType == "Add" ? "ADD" : "REMOVE",
              user_id: this.actionType == "Add" ? this.selectTeam : userId,
            },
          })
        );
    ...
    fetch_details() {
      var data = this.$store.getters.GetUserObj;
      var existingMembers = localStorage.getItem("channelMembers");
