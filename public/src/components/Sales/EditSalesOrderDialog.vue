  ...
  >
    <v-card class="pa-0 rounded-xl">
      <v-card-title class="d-flex align-center justify-space-between px-6 py-4">
        <span class="font-weight-bold text-h5">Edit Sales Order</span>
  ...
  methods: {
    loadSalesOrderData() {
      ...
      if (this.salesOrderData && Object.keys(this.salesOrderData).length > 0) {
        ...
        this.orderForm.customer_id = this.salesOrderData.customer_id || "";

        // Set customer search with full_user_name format
        if (this.salesOrderData.customer_id) {
          const customerInUpdatedList = this.masterCustomersupdated.find(
            (customer) =>
              customer.customer_id === this.salesOrderData.customer_id
          );
          this.orderForm.customer_search = customerInUpdatedList
            ? customerInUpdatedList.full_user_name
            : this.salesOrderData.customer_name || "";
        } else {
        ...
        this.orderForm.billing_name =
          ...
          this.salesOrderData.customer_name ||
          "";
        this.orderForm.project_name = this.salesOrderData.project_id;
        // Set project name with display ID format
        ...
        this.orderForm.round_off = this.salesOrderData.round_off == "true" ? true : false;
        this.orderForm.round_off_amount = this.salesOrderData.round_off_amount || "0.00";
        // console.log("this.orderForm.customer_id", this.orderForm.project_name);
    ...
    onCustomerSelect(customerId, type) {
      ...
      if (customerId) {
        const selectedCustomer = this.masterCustomersupdated.find(
          (customer) => customer.customer_id === customerId
        );
        if (selectedCustomer) {
          this.orderForm.customer_search = selectedCustomer.full_user_name;
          this.orderForm.customer_id = selectedCustomer.customer_id;

          // Find the original customer data for additional fields
          const originalCustomer = this.masterCustomers.find(
            (customer) => customer.customer_id === customerId
          );

          if (originalCustomer) {
            this.orderForm.phone_no = originalCustomer.phone_number || "";
            this.orderForm.billing_name =
              originalCustomer.billing_name ||
            ...
            let masterProjectsUpdate = this.masterProjects.filter(
              (project) =>
                ...
                originalCustomer.customer_email_id
            );
    ...
    async updateSalesOrder() {
      ...
      if (valid) {
        ...
        try {
          ...
          this.$emit(
            "errorMsg",
            error.errors[0].message || "An error occurred while updating the sales order"
          );
        } finally {
          this.loading = false;
        }
