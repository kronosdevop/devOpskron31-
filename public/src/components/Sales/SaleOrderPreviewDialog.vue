      ...
      <v-card class="order-preview-card">
        <v-toolbar elevation="0" dark dense class="navBar">
          <v-toolbar-title class="text--black ml-2">
            <div class="custom-title">Sales Order Preview</div>
          </v-toolbar-title>
          <v-spacer />
        ...
        <v-card-text class="pa-0">
          ...
          <div v-if="!hasOrderData" class="loading-state">
            <div class="text-center py-8">
              <v-progress-circular
                ...
                class="mb-4"
              ></v-progress-circular>
              <div class="text-body-2 text-grey">Loading sales order...</div>
            </div>
          </div>
          ...
          <div v-else class="order-content" ref="orderContent">
            <!-- Header -->
            <table class="order-header-table" border="1" cellspacing="0" cellpadding="8">
              <tbody>
                <tr>
                  <td class="header-left-cell" :rowspan="selectedOrder.due_date ? 4 : 3">
                  ...
                  <td class="header-right-cell order-title-cell">
                ...
                <tr v-if="selectedOrder.due_date">
                  <td class="header-right-cell detail-row-cell">
                    <div class="detail-row-content">
                    ...
                    <td class="customer-info-cell">
                      <div class="customer-info">
                        <h3 class="customer-name">
                          {{ selectedOrder.customer_name || selectedOrder.party_name || "N/A" }}
                        </h3>
                        <div class="customer-details-list">
                          <div class="customer-detail-row" v-if="selectedCustomer?.customer_display_id || selectedOrder.customer_code">
                            <span class="customer-detail-label">Customer ID#:</span>
                            <span class="customer-detail-value">{{ selectedCustomer?.customer_display_id || selectedOrder.customer_code }}</span>
                          </div>
                          <div class="customer-detail-row" v-if="selectedCustomer?.address_line_1 || selectedOrder.customer_address || selectedOrder.address_line_1">
                            <span class="customer-detail-label">Address:</span>
                            <span class="customer-detail-value">{{ selectedCustomer?.address_line_1 || selectedOrder.customer_address || selectedOrder.address_line_1 }}</span>
                          </div>
                          <div class="customer-detail-row" v-if="selectedCustomer?.primary_contact_no || selectedOrder.customer_phone">
                            <span class="customer-detail-label">Phone:</span>
                            <span class="customer-detail-value">{{ formatPhoneNumber(selectedCustomer?.primary_contact_no || selectedOrder.customer_phone) }}</span>
                          </div>
                          <div class="customer-detail-row" v-if="selectedCustomer?.gst_no || selectedOrder.customer_gst">
                            <span class="customer-detail-label">GST:</span>
                            <span class="customer-detail-value">{{ selectedCustomer?.gst_no || selectedOrder.customer_gst }}</span>
                          </div>
                          <div class="customer-detail-row" v-if="selectedCustomer?.state_1 || selectedOrder.customer_state">
                            <span class="customer-detail-label">State:</span>
                            <span class="customer-detail-value">{{ selectedCustomer?.state_1 || selectedOrder.customer_state }}</span>
                          </div>
                          <div class="customer-detail-row" v-if="selectedOrder.project_name">
                            <span class="customer-detail-label">Project:</span>
                            <span class="customer-detail-value">{{ selectedOrder.project_name }}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td class="reference-info-cell">
                      <div class="reference-section">
                        <div class="reference-content">
                          <div v-if="selectedOrder.reference">{{ selectedOrder.reference }}</div>
                          <div v-if="selectedOrder.comments">{{ selectedOrder.comments }}</div>
                          <div v-if="selectedOrder.instructions">{{ selectedOrder.instructions }}</div>
                          <div v-if="!selectedOrder.reference && !selectedOrder.comments && !selectedOrder.instructions" class="empty-reference">
                            &nbsp;
                          </div>
                        </div>
                      </div>
            ...
            <table class="items-table-combined" border="1" cellspacing="0" cellpadding="8">
              ...
              <tbody>
                ...
                <tr>
                  <td :colspan="itemConfiguration.items_wise_discount ? 7 : 6" :rowspan="summaryRowspan" valign="top" class="notes-cell">
                    <div class="notes-section">
                      <strong v-if="selectedOrder.description">Note:</strong>
                      <div class="notes-content">
                        <div v-if="selectedOrder.description">
                          <template v-for="(line, idx) in formatNotes(selectedOrder.description)" :key="idx">
                            {{ idx + 1 }}. {{ line }}<br v-if="idx < formatNotes(selectedOrder.description).length - 1">
                          </template>
                        </div>
                ...
                <tr v-if="itemConfiguration.items_wise_discount && selectedOrder.discount_percent && parseFloat(selectedOrder.discount_percent) > 0">
                  <td align="left" class="summary-label-cell">Discount</td>
                  <td align="right" class="summary-value-cell">-{{ getCurrencySymbol() }}{{ formatAmount(selectedOrder.discount_amount || '0.00') }}</td>
                </tr>

                <tr v-if="selectedOrder.tax_type && selectedOrder.tax_type !== 'Exempt'">
                  <td align="left" class="summary-label-cell">Tax</td>
                  <td align="right" class="summary-value-cell">{{ getCurrencySymbol() }}{{ formatAmount(selectedOrder.tax_amount || '0.00') }}</td>
                </tr>

                <tr v-if="selectedOrder.round_off && parseFloat(selectedOrder.round_off_amount || '0') !== 0">
                  <td align="left" class="summary-label-cell">Round Off</td>
                  <td align="right" class="summary-value-cell">{{ getCurrencySymbol() }}{{ formatAmount(selectedOrder.round_off_amount || '0.00') }}</td>
                </tr>
...
export default {
  ...
  computed: {
    ...
    isDataReady() {
      // Check if order data exists and categories API call is complete
      return this.hasOrderData && !this.isLoadingCategories;
    },
    selectedOrder() {
    ...
    selectedCustomer() {
      ...
      if (!this.selectedOrder?.customer_name || !this.masterCustomers || this.masterCustomers.length === 0) {
        return null;
      }
      return this.masterCustomers.find(
        (customer) => customer.legal_name === this.selectedOrder.customer_name
      ) || null;
    },
    orderItems() {
    ...
    summaryRowspan() {
      ...
      if (this.itemConfiguration.items_wise_discount && this.selectedOrder.discount_percent && parseFloat(this.selectedOrder.discount_percent) > 0) count++;
      if (this.selectedOrder.tax_type && this.selectedOrder.tax_type !== 'Exempt') count++;
      if (this.selectedOrder.round_off && parseFloat(this.selectedOrder.round_off_amount || '0') !== 0) count++;
      return count;
    },
  },
  ...
  methods: {
    ...
    async printOrder() {
      ...
      let scrollableContainer = orderElement;
      ...
      const allImages = orderElement.querySelectorAll("img");
      ...
      const vImgWrappers = orderElement.querySelectorAll(".v-img");
      ...
      printWindow.document.write(`
        <html>
          <head>
            <title>Sales Order - ${this.selectedOrder.invoice_no || 'N/A'}</title>
            <style>
              * {
