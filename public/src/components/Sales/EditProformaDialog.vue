<template>
    ...
    >
      <v-card class="pa-0 rounded-xl">
        <v-card-title class="d-flex align-center justify-space-between px-6 py-4">
          <span class="font-weight-bold text-h5">Edit Proforma</span>
          ...
          <v-form ref="form">
              ...
              <v-col cols="12" md="3">
                ...
                <div class="mb-3">
                  <v-text-field
                    ...
                    label="Proforma Date (DD/MM/YYYY)"
                    variant="outlined"
                    density="compact"
            ...
            <div class="mb-4">
              <div class="d-flex align-center justify-space-between mb-2">
                <h3 class="text-h6 font-weight-medium">Proforma Items</h3>
                <v-btn
                  color="primary"
                  size="small"
                  ...
                  <v-table density="comfortable" class="modern-table">
                    <thead>
                      <tr class="table-header">
                        ...
                        <th class="text-left pa-3" v-if="itemConfiguration.description" style="max-width: 140px; width: 140px;">DESCRIPTION</th>
                    ...
                    <tbody>
                      <tr v-for="(item, index) in orderForm.items" :key="index" class="table-row">
              ...
              <v-col cols="12" md="6">
                ...
                <div class="mb-3" v-if="transactionConfig.with_or_without_tax">
                  <div class="d-flex align-center justify-end">
                    ...
                    />
                    <span class="text-body-2 mr-4">{{ orderForm.tax_amount || '0' }}</span>
                  </div>
                </div>
                ...
                <div class="mb-3">
                  <div class="d-flex align-center">
                    ...
                    />
                    <span class="text-body-2 mr-2" v-if="transactionConfig.round_of">Round Off</span>
                    <v-text-field
                      v-if="transactionConfig.round_of"
        ...
        <v-card-text class="px-4 py-3">
          <v-textarea
            ...
            density="compact"
            rows="4"
            placeholder="Enter terms and conditions for this proforma..."
            hide-details
            class="mb-2"
          />
        </v-card-text>
  ...
  export default {
    ...
    methods: {
      ...
      loadProformaData() {
        // console.log("Proforma Data:", this.proformaData);      
        // console.log("Table Data Type:", typeof this.proformaData.table_data);
        // console.log("Table Data:", this.proformaData.table_data);
        
        if (this.proformaData && Object.keys(this.proformaData).length > 0) {
          // Load proforma data into the form
          this.orderForm.proforma_id = this.proformaData.proforma_id || "";
          this.orderForm.customer_id = this.proformaData.customer_id || "";
          this.orderForm.customer_search = this.proformaData.customer_name || "";
          this.orderForm.billing_name = this.proformaData.customer_name || "";
          this.orderForm.project_name = this.proformaData.project_id || "";
          this.orderForm.prefix_name = this.proformaData.prefix_name || "";
          this.orderForm.order_count = this.proformaData.order_count || "";
          this.orderForm.order_no = this.proformaData.invoice_no || "";
          this.orderForm.order_date = this.proformaData.invoice_date || this.getCurrentDate();
          this.orderForm.description = this.proformaData.description || "";
          this.orderForm.terms_conditions = this.proformaData.terms_conditions || "";
          if( this.orderForm.description){
            this.showDescription = true;
          }
          this.orderForm.state_of_supply = this.proformaData.state || "";
          this.orderForm.discount_percent = this.proformaData.discount_percent || "";
          this.orderForm.discount_amount = this.proformaData.discount_amount || "0.00";
          this.orderForm.tax_type = this.proformaData.tax_type || "";
          this.orderForm.tax_amount = this.proformaData.tax_amount || "0.00";
          this.orderForm.grand_total = this.proformaData.total_cost || "0";
          this.orderForm.transaction_id = this.proformaData.transaction_id || "";
          this.orderForm.po_number = this.proformaData.po_no || "";
          this.orderForm.round_off = this.proformaData.round_off == "true" ? true : false;
          this.orderForm.round_off_amount = this.proformaData.round_off_amount || "0.00";
          this.onCustomerSelect(this.orderForm.customer_id);
          // Load items if available
          if (this.proformaData.table_data) {
            try {
              // Check if table_data is already an array or needs to be parsed
              let items;
              if (Array.isArray(this.proformaData.table_data)) {
                items = this.proformaData.table_data;
              } else if (typeof this.proformaData.table_data === 'string') {
                items = JSON.parse(this.proformaData.table_data);
              } else {
                items = [];
              }
          ...
          if (this.proformaData.image_url) {
            this.orderForm.originalImageUrl = this.proformaData.image_url;
            this.showImageUpload=true
            // Fetch signed URL for display
            this.fetchImageForDisplay(this.proformaData.image_url);
          }

          // Load payment details if available
          if (this.proformaData.payment_details) {
            try {
              let payments;
              // Handle both string and object cases
              if (typeof this.proformaData.payment_details === 'string') {
                payments = JSON.parse(this.proformaData.payment_details);
              } else {
                payments = this.proformaData.payment_details;
              }
    ...
    async uploadImage() {
      ...
      try {
        ...
        const key = [
          ...
          `${Date.now()}__${this.orderForm.imageFile.name.replace(
            /\s+/g,
            ""
          )}`,
        ].join("/");
      ...
      async updateProforma() {
        ...
        if (valid) {
          ...
          try {
            ...
            // console.log("Updated proforma data:", this.orderForm);
            ...
            this.$emit(
              "errorMsg",
              error.errors[0].message || "An error occurred while updating the proforma"
            );
          } finally {
            this.loading = false;
          }
