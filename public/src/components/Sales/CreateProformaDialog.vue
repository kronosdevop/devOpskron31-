    ...
    <v-card class="pa-0 rounded-xl">
      <v-card-title class="d-flex align-center justify-space-between px-6 py-4">
        <span class="font-weight-bold text-h5">{{
          isEditMode ? "Convert to Proforma" : "Create Proforma"
        }}</span>
...
export default {
    ...
    populateFormFromQuotation(quotation) {
      try {
        ...
        if (quotation.customer_id) {
          this.orderForm.customer_id = quotation.customer_id;
          this.orderForm.customer_search = quotation.customer_name || "";
          this.orderForm.billing_name = quotation.customer_name || "";
        }

        // Populate project details
        if (quotation.project_id) {
          this.orderForm.project_name = quotation.project_id;
        }

        // Populate PO number
        if (quotation.po_no) {
          this.orderForm.po_number = quotation.po_no;
        }
        if (quotation.round_off) {
          this.orderForm.round_off =
            quotation.round_off == "true" ? true : false;
        }
        // console.log("quotation.round_off", quotation.round_off);
        if (quotation.round_off_amount) {
          this.orderForm.round_off_amount =
            quotation.round_off_amount || "0.00";
        }
        // console.log("this.orderForm.round_off_amount", this.orderForm.round_off_amount);

        // Populate date
        if (quotation.invoice_date) {
          this.orderForm.order_date = quotation.invoice_date;
        }

        // Populate state of supply
        if (quotation.state) {
          this.orderForm.state_of_supply = quotation.state;
        }

        // Populate description
        if (quotation.description) {
          this.orderForm.description = quotation.description;
        }

        // Populate terms and conditions
        if (quotation.terms_conditions) {
          this.orderForm.terms_conditions = quotation.terms_conditions;
        }
        if (quotation.transaction_id) {
          this.orderForm.refrence_id = quotation.transaction_id;
        }
        // console.log("quotation", quotation);W
        // Populate items
        if (quotation.table_data) {
          try {
            const itemsData = quotation.table_data;
            if (Array.isArray(itemsData)) {
              this.orderForm.items = itemsData.map((item) => ({
            ...
            console.error("Error parsing table data:", parseError);
          }
        }

        // Populate totals
        if (quotation.total_cost) {
          this.orderForm.grand_total = quotation.total_cost;
        }

        if (quotation.discount_percent) {
          this.orderForm.discount_percent = quotation.discount_percent;
        }

        if (quotation.discount_amount) {
          this.orderForm.discount_amount = quotation.discount_amount;
        }

        if (quotation.tax_type) {
          this.orderForm.tax_type = quotation.tax_type;
        }

        if (quotation.tax_amount) {
          this.orderForm.tax_amount = quotation.tax_amount;
        }

        // Populate payments
        if (quotation.payment_details) {
          try {
            let payments;
            // Handle both string and object cases
            if (typeof quotation.payment_details === 'string') {
              payments = JSON.parse(quotation.payment_details);
            } else {
              payments = quotation.payment_details;
            }
        ...
        // console.log("this.orderForm.round_off_amount in populateFormFromQuotation", this.orderForm.round_off_amount);
  ...
  return this.masterItems.filter(item =>
    ...
    calculateOrderTotals() {
      ...
      // console.log("this.orderForm.round_off", this.orderForm.round_off);
      // console.log("this.orderForm.round_off_amount", this.orderForm.round_off_amount);
    ...
    async saveOrder() {
      ...
      if (valid) {
        ...
        // if (this.orderForm.items.length !== 1) {
        //   this.$emit("errorMsg", "Please ensure there is exactly one item in the proforma");
        //   return;
        // }
        ...
        try {
          ...
          // console.log("Proforma data:", this.orderForm);
          ...
          if (result.Status === "SUCCESS") {
            this.$emit(
              "successMsg",
              this.isEditMode
                ? "Quotation successfully converted to Proforma"
          ...
          this.$emit(
            "errorMsg",
            error.errors[0].message || "An error occurred while creating the proforma"
          );
        } finally {
          this.loading = false;
        }
